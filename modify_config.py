...
if os.path.exists(lock_file_path):
    with open(lock_file_path, 'r', encoding='utf-8') as f:
        ...
        if "-" in content:
            ...
            saved_code = content [cite: 434, 435]
...
if is_reset_day and saved_month != current_month:
    ...
    if not saved_code or len(saved_code) != 3 or "-" not in (content if os.path.exists(lock_file_path) else ""):
        current_token = ''.join(random.choices(string.ascii_lowercase + string.digits, k=3))
        with open(lock_file_path, 'w', encoding='utf-8') as f:
            f.write(f"{current_month}-{current_token}") [cite: 435, 436]
    else:
        current_token = saved_code
    print(f"📡 正常沿用本月密鎖後綴: {current_token}") [cite: 436]
...
print(f"Target file path -> {output_path}") [cite: 436]
    ...
    if os.path.basename(old_file) != output_filename:
        try:
            ...
            with open(old_file, 'w', encoding='utf-8') as f:
                json.dump(trap_json, f, ensure_ascii=False, indent=4) [cite: 439]
        except: pass

for garbage in glob.glob('datas/config_*.json'):
    try: os.remove(garbage)
    except: pass [cite: 439]
    ...
    if "🔞" in item.get("name", ""):
        ...
        if "api" in item and isinstance(item["api"], str):
            if item["api"].startswith("./py/"):
                item["api"] = item["api"].replace("./py/", "https://gh-proxy.com/https://raw.githubusercontent.com/ediart/tvbox/refs/heads/main/lz/py/") [cite: 440]
            elif item["api"].startswith("./js/"):
                item["api"] = item["api"].replace("./js/", "https://gh-proxy.com/https://raw.githubusercontent.com/ediart/tvbox/refs/heads/main/lz/js/") [cite: 441]
...
else: haitun_lives.append(country_live_dict) [cite: 442]

# ====================================================================
# 🎯 雙私房爬蟲線路注入：無縫融合 JavBus 與 MissAV 
# ====================================================================
javbus_site_dict = {
    "key": "JavBus_Private",
...
# 陣列大合併：在海豚和老張源中間插入雙爬蟲專線 
json_cnb["sites"] = haitun_sites + [javbus_site_dict, missav_site_dict] + lz_nsfw_list + json_cnb.get("sites", []) [cite: 442]
json_cnb["lives"] = haitun_lives + json_cnb.get("lives", []) [cite: 442]

# 文本清洗 
final_json_text = json.dumps(json_cnb, ensure_ascii=False, indent=4) [cite: 442]
final_json_text = final_json_text.replace('"key": "hajim-腾讯备"', '"spider": "./tvbox.jar",\n            "key": "hajim-腾讯备"') [cite: 442, 443]
final_json_text = final_json_text.replace('"key": "茫茫"', '"spider": "./tvbox.jar",\n            "key": "茫茫"') [cite: 443]
final_json_text = final_json_text.replace('🐬', '').replace('海豚影视', '').replace('海豚', '') [cite: 443]
final_json_text = final_json_text.replace('完全免费，如有收费的都是骗子', '').replace('交流群 TG：@hshsjk9', '') [cite: 443]
...
for src, dst in path_replacements.items():
    final_json_text = final_json_text.replace(src, dst) [cite: 443]

thanks_warning = "👑 特别致谢与版权声明\n🐋 感谢鱼佬与海豚佬的付出！" [cite: 443]
welcome_notice = "👑 欢迎使用【老杨TV粉丝专属缝合专线】！🚨 核心秒播優化版已啟用：網絡棧已實裝 DoH 加密防污染與超級並行解析專線，已成功無縫融合 JavBus 旗艦與 MissAV 女優榜兩大私房專線！" [cite: 443]

try:
    final_obj = json.loads(final_json_text) [cite: 443, 444]
    ...
    parallel_parse = {"name": "🚀 老楊專屬 • 聚合秒播專線 (超級並行)", "type": 4, "url": "Parallel"} [cite: 444]
    final_obj["parses"] = [parallel_parse] + final_obj.get("parses", []) [cite: 444]
    
    for site in final_obj.get("sites", []): [cite: 444, 445]
        if site.get("key") == "leo_danmaku":
            site["indexs"] = 1
            if "index" in site: site.pop("index") [cite: 445]
        if site.get("type") == 1 and "filterable" in site:
            site.pop("filterable") [cite: 445]

    for live in final_obj.get("lives", []): [cite: 445]
        lname = live.get("name", "")
        if "綜合直播" in lname or "地方直播" in lname or "各大源合集" in lname: [cite: 445, 446]
            live["epg"] = "https://epg.112114.xyz/?ch={name},https://epg.pw/xmltv/assets/china.xml.gz"
            live["timeZone"] = "Asia/Shanghai" [cite: 446]
        elif "台灣台" in lname or "港台" in lname:
            live["epg"] = "https://epg.pw/xmltv/assets/taiwan.xml.gz"
            live["timeZone"] = "Asia/Taipei" [cite: 446]

    for player in final_obj.get("ijk", []): [cite: 446]
        for option in player.get("options", []): [cite: 446, 447]
            if option.get("name") == "dns_cache_timeout":
                option["value"] = "60000000" [cite: 447]

    final_obj["notice"] = welcome_notice
    final_obj["warningText"] = thanks_warning [cite: 447]
    
    ordered_obj = {}
    if "notice" in final_obj: ordered_obj["notice"] = final_obj.pop("notice") [cite: 447]
    if "warningText" in final_obj: ordered_obj["warningText"] = final_obj.pop("warningText") [cite: 447]
    ordered_obj.update(final_obj) [cite: 447]
    
    try:
        for site in ordered_obj.get("sites", []): [cite: 447]
            if "name" in site: [cite: 448]
                name_val = site["name"]
                for char in ['丨', '┃', ' ']: name_val = name_val.strip(char) [cite: 448]
                name_val = re.sub(r'\s+', ' ', name_val) [cite: 448]
                if not name_val.startswith("🦋"): site["name"] = f"🦋 {name_val}" [cite: 448, 449]
        for site in ordered_obj.get("sites", []): [cite: 449]
            if "key" in site and site["key"] == "AQY":
                site["name"] = "🦋 爱奇艺｜此接口非原创，合并自海豚佬 and 鱼佬接口，感谢两位大佬的付出，如有侵权，联系删除｜@huliys9" [cite: 449]
    except: pass

    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(ordered_obj, f, ensure_ascii=False, indent=4) [cite: 449]
        
    with open(tracker_path, 'w', encoding='utf-8') as f:
        f.write(output_filename) [cite: 449, 450]
        
    print(f"🎉 全量版秒播優化配置更新成功！檔案已寫出至: {output_path}") [cite: 450]

except Exception as e:
    print(f"❌ 嚴重錯誤：最後的本地渲染失敗，原因: {e}") [cite: 450]
...
if not os.path.exists(lock_file_path) or "-" not in (open(lock_file_path, 'r', encoding='utf-8').read() if os.path.exists(lock_file_path) else ""):
    with open(lock_file_path, 'w', encoding='utf-8') as f:
        f.write(f"{current_month}-{current_token}") [cite: 450]
