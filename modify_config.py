    ...
    try:
        ...
        for site in ordered_obj.get("sites", []):
            if "name" in site:
                ...
                if not name_val.startswith("🦋"): name_val = f"🦋 {name_val}"
                site["name"] = name_val
                ...
                if s_genre == "shortdrama" or "短剧" in name_val or "dj" in s_key.lower(): site["category"] = "短剧"
                    ...
                    site["category"] = "音乐"
                    site["searchable"] = 0
                elif "动漫" in name_val or "新番" in name_val or "Anime" in s_key: site["category"] = "动漫"
                elif "磁力" in name_val or "索" in name_val or "盘" in name_val or "云盘" in name_val or "4K" in name_val: site["category"] = "网盘/磁力"
                elif "体育" in name_val or "球" in name_val or "直播" in name_val: site["category"] = "体育/直播"
                else: site["category"] = "综合"

        for site in ordered_obj.get("sites", []):
            if "key" in site and site["key"] == "AQY":
                site["name"] = "🦋 爱奇艺｜此接口非原创，感谢大佬的付出｜@huliys9"
                site["category"] = "综合"
    except: pass
